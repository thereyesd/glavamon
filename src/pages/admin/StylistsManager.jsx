...
export default function StylistsManager() {
  ...
  async function handleDelete(id) {
    if (!window.confirm('¿Estás seguro de eliminar este estilista?')) return
    try {
      ...
      <div className="flex border-t border-surface-border/50">
        ...
        <button
          ...
          className={`flex-1 py-3 text-sm transition-colors flex items-center justify-center gap-1 ${
            stylist.isActive ? 'text-primary' : 'text-gray-400 hover:text-white hover:bg-surface-highlight/50'
          }`}
        >
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-background-dark rounded-t-3xl border-t border-surface-border max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">URL de Foto</label>
            <input
              type="url"
